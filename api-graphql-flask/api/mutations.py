...
def resolve_create_todo(obj, info, description, due_date):
  try:
    ...
    payload = {
      "success": False,
      "error": [f"Incorrect date format provided. Date should be in the format dd-mm-yyyy"]
    }
...
def resolve_mark_done(obj, info, todo_id):
  try:
    ...
    payload = {
      "success": False,
      "error": [f"Todo motching id {todo_id} was not found"]
    }
...
def resolve_delete_todo(obj, info, todo_id):
  try:
    ...
    payload = {
      "success": False,
      "errors": [f"Todo motching id {todo_id} not found"]
    }
